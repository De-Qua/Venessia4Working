# Per creare un grafo per il sito: - su qgis, convertire in epsg4326, spezzare i ponti (chissà come, ale lo sa) - da questo script, togliere mestre, togliere le colonne inutili, salvare in pickle e aggiornare il txt
...
########### creiamo il txt dei toponimi, togliendo mestre e cambiando le coordinate
TP = gpd.read_file(folder + "/../data_bkp" + "/TP_STR.shp")
TP_centroids_old_coord = TP["geometry"].centroid
cx = [centroid.x if centroid else 0. for centroid in TP_centroids_old_coord]
cy = [centroid.y if centroid else 0. for centroid in TP_centroids_old_coord]
TP = TP.to_crs(epsg=4326)
cx_new = [centroid.x if centroid else 0. for centroid in TP_centroids]
cy_new = [centroid.y if centroid else 0. for centroid in TP_centroids]
...
cx_new = np.asarray(cx_new)
cy_new = np.asarray(cy_new)
cx = np.asarray(cx)
cy = np.asarray(cy)
...
coordsx = cx_new[indices]
coordsy = cy_new[indices]
...
TP_coord_full = np.transpose(TP_coord_full)
